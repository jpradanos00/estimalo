  ...
  return (
    <form onSubmit={handleSubmit} className={`flex gap-2 ${compact ? 'py-1' : 'py-2'}`}>
      ...
      <button
        type="submit"
        disabled={!title.trim()}
        className="px-4 py-2 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-500 disabled:opacity-50 transition-colors focus-ring min-h-[40px] flex-shrink-0"
      >
      ...
      {onCancel && (
        <button
          type="button"
          onClick={onCancel}
          className="px-3 py-2 rounded-xl text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors focus-ring min-h-[40px]"
        >
          ✕
        </button>
      )}
